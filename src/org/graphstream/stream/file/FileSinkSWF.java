...
public class FileSinkSWF implements FileSink
{
	private static class BitString
	{
		...
		BitString( int ... values )
		{
			...
			for( int i = 0; i < values.length; i++ )
				bitsize = Math.max( bitsize, Integer.toBinaryString(Math.abs(values[i])).length() );
			...
			for( int i = sizeBinary.length() - 1; i > Math.max(0,sizeBinary.length() - 6); i-- )
			...
			for( int i = 0; i < values.length; i++ )
			{
				...
				{
					if( j < valueBinary.length() ) 	add( valueBinary.charAt(bitsize-1-j) != '0' );
					else							add( values [i] < 0 );
				}
			}
		}
		...
		void __resize( int size )
		{
			...
			for( int i = 0; i < Math.min( this.size, size ); i++ ) nBuffer.put( buffer.get( i ) );
		...
		public void
		fill( boolean v, int offset, int size )
		{
		...
		String toString()
		{
			...
			{
				for( int j = 0; j < 8; j++ ) b.append( valueAt( i * 8 + j ) ? '1' : '0' );
			}
			for( int i = 0; i < bitCurseur; i++ ) b.append( valueAt( curseur * 8 + i ) ? '1' : '0' );
			return b.toString();
		}
	}
	...
	private static void writeRECT(ByteBuffer buffer, int xmin,
			int xmax, int ymin, int ymax) {
		writeSingleBits(buffer,xmin,xmax,ymin,ymax);
	}

	private static void writeSingleBits(ByteBuffer buffer,
			int... values) {
