	...
	extends FileSinkBase
{
	...
	/**
	 * Output policy.
	 * Specify when an image is written. This is an important choice.
	 * Best choice is to divide the graph in steps and choose the
	 * *ByStepOutput*. Remember that if your graph has x nodes and
	 * *ByEventOutput* or *ByNodeEventOutput* is chosen, this will
	 * produce x images just for nodes creation.
	 */
	...
	/**
	 * Layout policy.
	 * Specify how layout is computed. It can be computed in its
	 * own thread with a LayoutRunner but if image rendering takes
	 * too much time, node positions will be very different between
	 * two images. To have a better result, we can choose to compute
	 * layout when a new image is rendered. This will smooth the move
	 * of nodes in the movie.
	 */
	...
	/**
	 * Experimental. 
	 * Allows to choose which renderer will be used.
	 */
	...
	public void setRenderer( RendererType rendererType )
	{
		try
		{
			Class<? extends GraphRenderer> clazz =
				(Class<? extends GraphRenderer>) Class.forName(rendererType.classname);
	...
	{
		image 	= new BufferedImage( resolution.getWidth(), resolution.getHeight(),
	...
	{
		gg.edgeAttributeChanged(sourceId, timeId, edgeId, attribute, oldValue, newValue);
	...
	{
		gg.graphAttributeChanged(sourceId, timeId, attribute, oldValue, newValue);
		...
		throws IOException
	{
		...
		FileSinkImages	fsi	= new FileSinkImages( args[1], OutputType.valueOf(args[2]), Resolutions.valueOf(args[3]), OutputPolicy.valueOf(args[4]) );
