 ...
 * 
 * <p>
 * This implementation is declined in two sub implementations
 * {@link SingleGraph} and {@link MultiGraph}.
 * </p>
 * 
 * <p>
 ...
 * <ul>
 * <li>automatically deletes edges connected to a node when this one disappears;
 * </li>
...
public class DefaultGraph extends AbstractElement implements Graph {
	...
	/**
	 * Helpful class that dynamically instantiate edges according to a given
	 * class name.
	 */
	...
	public DefaultGraph(String id, boolean strictChecking, boolean autoCreate) {
		...
		/*
		 * otherTimes = new SinkTime(); time = new SourceTime( id, otherTimes );
		 */
		...
		edgeFactory = new EdgeFactory<SingleEdge>() {
			public SingleEdge newInstance(String id, Node src, Node trg,
					boolean directed) {
	...
	public <T extends Node> T getNode(String id) {
		try {
			...
			System.err
					.printf("%s:%s:%d incompatible node type%n",
							elt.getFileName(), elt.getMethodName(),
							elt.getLineNumber());
		}
	...
	public <T extends Edge> T getEdge(String id) {
		try {
			...
			System.err
					.printf("%s:%s:%d incompatible edge type%n",
							elt.getFileName(), elt.getMethodName(),
							elt.getLineNumber());
		}
	...
	public void setEdgeFactory(EdgeFactory<? extends Edge> ef) {
		try {
			this.edgeFactory = (EdgeFactory<? extends DefaultEdge>) ef;
		} catch (ClassCastException e) {
	...
	public void setNodeFactory(NodeFactory<? extends Node> nf) {
		try {
			this.nodeFactory = (NodeFactory<? extends DefaultNode>) nf;
		} catch (ClassCastException e) {
	...
	protected <T extends Node> T addNode_(String sourceId, long timeId,
		...
		// System.err.printf( "%s.addNode_(%s, %d, %s)%n", getId(), sourceId,
		// timeId, nodeId );
	...
	protected <T extends Node> T removeNode_(String sourceId, long timeId,
			String nodeId, boolean fromNodeIterator)
		...
		if (node != null) // XXX probably to remove !!
		{
			...
			if (strictChecking)
				throw new ElementNotFoundException("node id '" + nodeId
	...
	protected <T extends Edge> T addEdge_(String sourceId, long timeId,
		...
		if (src != null && trg != null) {
			...
			{
				T edge = (T) src.addEdgeToward(edgeId, (DefaultNode) trg,
						directed);
				...
				throw new IdAlreadyInUseException("cannot add edge '" + edgeId
	...
	/**
	 * @complexity O(log(n+m)) with n being the number of nodes and m the number
	 *             of edges.
	 */
	...
	public <T extends Edge> T addEdge(String id, String from, String to,
		...
		// erased, its id is not remembered and the edge with id AB is
		// transformed
		// in undirected edge. Therefore, sometimes the id is not the same as
		// the
		// edge.getId(). Nevertheless only one edge exists and so no event must
		// be generated.
		// TODO: this strange behaviour should disappear ! Adding BA should
		// cause
	...
	protected <T extends Edge> T removeEdge_(String sourceId, long timeId,
			...
			if (src == null) {
				if (strictChecking)
					throw new ElementNotFoundException(
							"error while removing edge '" + from + "->" + to
									+ "' node '" + from + "' cannot be found");
			}
		...
		if (strictChecking)
			throw new ElementNotFoundException("no edge between nodes '" + from
					+ "' and '" + to + "'");
	...
	@SuppressWarnings("unchecked")
	protected <T extends Edge> T removeEdge_(String sourceId, long timeId,
			String edgeId, boolean fromEdgeIterator) {
		try {
			...
			if (strictChecking)
				throw new ElementNotFoundException(
						"illegal edge state while removing edge '" + edgeId
								+ "'");
		}

		// if( strictChecking )
		// throw new NotFoundException(
		// "edge '"+edgeId+"' does not exist, cannot remove" );
	...
	public void read(String filename) throws IOException, GraphParseException,
		...
		if (input != null) {
			...
			throw new ElementNotFoundException(String.format(
					"no reader for unknown format of file '%s'", filename));
		}
	}
	...
	public void write(String filename) throws IOException {
		...
		if (output != null)
			...
			throw new IOException(String.format(
					"no file output factory for unknown format of file %s",
					filename));
	}
	...
	/**
	 * An internal class that represent an iterator able to browse edge or node
	 * sets and to remove correctly elements. This is tricky since removing a
	 * node or edge does more than only altering the node or edge sets (events
	 * for example).
	 * 
	 * @param <T>
	...
	class ElementIterator<T extends Element> implements Iterator<T> {
		...
		/**
		 * The last browsed element via next(). This allows to get the element
		 * id to (eventually) remove it.
		 */
		T current;
		 ...
		 * 
		 * @param graph
		 *            The graph the set of elements pertains to, this reference
		 *            is used for removing elements.
		 * @param elements
		 *            The elements set to browse.
		 * @param onNodes
		 *            If true the set is a node set, else it is an edge set.
		 */
		ElementIterator(DefaultGraph graph,
				HashMap<String, ? extends T> elements, boolean onNodes) {
		 ...
		 *            If true the set is a node set, else it is an edge set.
		 */
		ElementIterator(DefaultGraph graph, ArrayList<T> elements,
				boolean onNodes) {
		...
		public void remove() {
			if (onNodes) {
				if (current != null) {
					graph.removeNode_(graph.getId(), listeners.newEvent(),
							current.getId(), true);
					iterator.remove();
				}
			} else // On edges.
			{
				if (current != null) {
					graph.removeEdge_(graph.getId(), listeners.newEvent(),
							current.getId(), true);
					iterator.remove();
				}
			}
	...
	class GraphListeners extends SourceBase implements Pipe {
		...
		public void edgeAttributeAdded(String sourceId, long timeId,
			...
			if (sinkTime.isNewEvent(sourceId, timeId)) {
				...
				if (edge != null)
					((DefaultEdge) edge).addAttribute_(sourceId, timeId,
							attribute, value);
			}
		}
		...
		public void edgeAttributeChanged(String sourceId, long timeId,
			...
			if (sinkTime.isNewEvent(sourceId, timeId)) {
				...
				if (edge != null)
					((DefaultEdge) edge).changeAttribute_(sourceId, timeId,
							attribute, newValue);
			}
		}
		...
		public void edgeAttributeRemoved(String sourceId, long timeId,
			...
			if (sinkTime.isNewEvent(sourceId, timeId)) {
				...
				if (edge != null)
					((DefaultEdge) edge).removeAttribute_(sourceId, timeId,
							attribute);
			}
		}
		...
		public void nodeAttributeAdded(String sourceId, long timeId,
			...
			if (sinkTime.isNewEvent(sourceId, timeId)) {
				...
				if (node != null)
					((DefaultNode) node).addAttribute_(sourceId, timeId,
							attribute, value);
			}
		}
		...
		public void nodeAttributeChanged(String sourceId, long timeId,
			...
			if (sinkTime.isNewEvent(sourceId, timeId)) {
				...
				if (node != null)
					((DefaultNode) node).changeAttribute_(sourceId, timeId,
							attribute, newValue);
			}
		}
		...
		public void nodeAttributeRemoved(String sourceId, long timeId,
			...
			if (sinkTime.isNewEvent(sourceId, timeId)) {
				...
				if (node != null)
					((DefaultNode) node).removeAttribute_(sourceId, timeId,
							attribute);
			}
		}
		...
		public void nodeAdded(String sourceId, long timeId, String nodeId) {
			// System.err.printf( "%s.nodeAdded( %s, %d, %s ) =>", getId(),
