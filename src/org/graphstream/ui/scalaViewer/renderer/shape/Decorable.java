...
import org.graphstream.ui.scalaViewer.renderer.shape.swing.ShapeDecor;
...
public class Decorable extends HasSkel {
	...
	public ShapeDecor theDecor = null ;
 	...
 	public void decorArea(Backend backend, ScalaDefaultCamera camera, IconAndText iconAndText, GraphicElement element, java.awt.Shape shape ) {
	...
	public void decorConnector(Backend backend, ScalaDefaultCamera camera, IconAndText iconAndText, GraphicElement element, java.awt.Shape shape ) {
		boolean visible = true ;
 	  	if( element != null ) visible = camera.isTextVisible( element );
 	  	if( theDecor != null && visible ) {
	...
	/** Configure all the static parts needed to decor the shape. */
  	public void configureDecorableForGroup( Style style, ScalaDefaultCamera camera) {
		/*if( theDecor == null )*/ theDecor = ShapeDecor.apply( style );
  	}
  	/** Setup the parts of the decor specific to each element. */
  	public void configureDecorableForElement(Backend backend, ScalaDefaultCamera camera, GraphicElement element, Skeleton skel) {
  		text = element.label;
  		if( skel != null ) {
